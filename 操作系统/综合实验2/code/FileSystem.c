...
typedef struct USEROPEN {
    ...
    char dir
        [80];  // 打开文件所在路径，以便快速检查指定文件是否已经打开。怎么检查
    int count;      // 读写指针的位置
...
char filesys_name[] = "virtual_disk.img";  // 文件系统文件的文件名
unsigned char* buf;
...
int do_write(int fd,
             char* text,
             int len,
             char wstyle);  // 将缓冲区中的内容写到指定文件中
int myread(
    int fd,
    int len);  // 读出指定文件中从读写指针开始的长度为len的内容到用户空间中
int do_read(
    int fd,
    int len,
    char*
        text);  // 读出指定文件中从读写指针开始的长度为len的内容到用户空间的text中
...
void startsys() {
    ...
    if ((fp = fopen(filesys_name, "r")) == NULL) {
        printf(
            "The file system doesn't exist, and then it will be created by "
            "myformat().\n");
        my_format();
    } else {
        printf(
            "The file system exists, and then it will be written to "
            "Myvhard.\n");
...
void my_format() {
    ...
    // 初始化引导块
    strcpy(guide->information,
           "Every block's size is 1024 bytes,and the number of the block is "
           "1000.");
    guide->root = 5;
    ...
    */
    ...
    root->time =
        lc_time->tm_hour * 2048 + lc_time->tm_min * 32 + lc_time->tm_sec / 2;
    root->date = (lc_time->tm_year - 80) * 512 + (lc_time->tm_mon + 1) * 32 +
                 lc_time->tm_mday;
    root->first = 5;
    ...
    */
    ...
    /*
        size_t fwrite ( const void * ptr, size_t siz e, size_t count, FILE *
       stream ); params: ptr：指向要被写入的数据 size：每组数据的大小；
            count：数据的个数
            stream：输出文件的指针
...
void my_mkdir(char* dirname) {
    ...
    fcbptr->time =
        lc_time->tm_hour * 2048 + lc_time->tm_min * 32 + lc_time->tm_sec / 2;
    fcbptr->date = (lc_time->tm_year - 80) * 512 + (lc_time->tm_mon + 1) * 32 +
                   lc_time->tm_mday;
    ...
    // printFcb(father);
    for (i = 0; i < (num / sizeof(fcb)); i++) {
        if ((!strcmp(fcbptr->filename, openfile[curdir].filename)) &&
            (!strcmp(fcbptr->exname, openfile[curdir].exname))) {
            fcbptr->length = openfile[curdir].length;
            break;
        }
        fcbptr++;
        ...
        if (fcbptr->free) {
            if (!fcbptr->attribute)
                printf("%s\\\t\t<DIR>\t\t%d/%d/%d\t%02d:%02d:%02d\n",
                       fcbptr->filename, (fcbptr->date >> 9) + 1980,
                       (fcbptr->date >> 5) & 0x000f, fcbptr->date & 0x001f,
                       fcbptr->time >> 11, (fcbptr->time >> 5) & 0x003f,
                       fcbptr->time & 0x001f * 2);
            else {
                printf("%s.%s\t\t%dB\t\t%d/%d/%d\t%02d:%02d:%02d\t\n",
                       fcbptr->filename, fcbptr->exname, (int)(fcbptr->length),
                       (fcbptr->date >> 9) + 1980, (fcbptr->date >> 5) & 0x000f,
                       fcbptr->date & 0x001f, fcbptr->time >> 11,
                       (fcbptr->time >> 5) & 0x3f, fcbptr->time & 0x1f * 2);
            }
        }
        fcbptr++;
    }
...
int my_create(char* dirname) {
    ...
    char *fname, *exname;
    char dest_dir[80] = "", filename[20] = "", text[MAXTEXT] = "",
         dir_tmp[80] = "";  // 要创建文件的目录
    ...
    for (i = 0; i < (num / sizeof(fcb)); i++) {
        // 查找是否有同名文件
        if ((!strcmp(pos->filename, fname)) &&
            (!(strcmp(pos->exname, exname)))) {
            printf("The filename to create has been exist!\n");
            return ERROR;
        }
        pos++;
    ...
    pos->time =
        lc_time->tm_hour * 2048 + lc_time->tm_min * 32 + lc_time->tm_sec / 2;
    pos->date = (lc_time->tm_year - 80) * 512 + (lc_time->tm_mon + 1) * 32 +
                lc_time->tm_mday;
    pos->first = blk_num;
    ...
    // printFcb(father);
    for (i = 0; i < (num / sizeof(fcb)); i++) {
        if ((!strcmp(pos->filename, openfile[curdir].filename)) &&
            (!strcmp(pos->exname, openfile[curdir].exname))) {
            pos->length = openfile[curdir].length;
            break;
        }
        pos++;
...
void my_rm(char* dirname) {
    ...
    pos = (fcb*)text;
    for (i = 0; i < (num / sizeof(fcb)); i++) {
        if (strcmp(pos->filename, fname) == 0 &&
            strcmp(pos->exname, exname) == 0)
            break;
        pos++;
    }
    if (i == num / sizeof(fcb)) {
...
int my_open(char* dirname) {
    ...
    char dest_dir[80] = "", filename[20] = "",
         text[MAXTEXT];  // 要创建文件的目录
        ...
        /*
            ...
            return:
                value:
                    ...
                    if s1 > s2 : values > 0;
        */
        if (!strcmp(fname, openfile[i].filename) &&
            !strcmp(exname, openfile[i].exname)) {
            return i;
        }
    }
    // 查看父目录中的fcb
...
void my_close(int fd) {
    ...
    if (openfile[fd].fcbstate) {
        ...
        // 定位到文件对应的目录项
        for (i = 0; i < (num / sizeof(fcb)); i++) {
            if (strcmp(pos->filename, openfile[fd].filename) == 0 &&
                strcmp(pos->exname, openfile[fd].exname) == 0)
                break;
            pos++;
        }
        openfile[father].count = i * sizeof(fcb);
...
int my_write(int fd) {
    ...
    while (1) {
        printf(
            "Please choose the number of the style that the order file be "
            "written:\n1.截断写\t2.覆盖写\t3.追加写\n");
        scanf("%d", &wstyle);
    ...
    // printf("In my_write, the fd is %d, the filename is %s,the length is
...
void printOpenFile() {
    ...
    for (i = 0; i < MAXOPENFILE; i++) {
        if (openfile[i].topenfile != 0) {
            ...
            printf("%d\t\t%s.%s\t\t%d\t\t%s\t\t%d\n", i, openfile[i].filename,
                   openfile[i].exname, openfile[i].length,
                   openfile[father].filename, father);
        }
    }
}
...
int main() {
    char cmd[15][10] = {"cd",     "mkdir", "rmdir", "ls",    "create",
                        "rm",     "open",  "close", "write", "read",
                        "format", "exit",  "rm-r",  "popen"};
    char s[30], *sp;
